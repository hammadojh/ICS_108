# ICS 108 Lecture 6 — User-defined methods

A roughly 30-minute tutorial in the accepted three-problem format. Every concept uses the same Pn-Cn ID across code, supporting slides, the complete-code handout, and the instructor refresher.
...
- [Content map](docs/CONTENT_MAP.md): all 18 source sections, selected scope, timing, and deferred topics.

Open the HTML files directly in a browser. No network connection or external assets are required. In supporting slides, use Arrow/Page keys, Space/Enter, Backspace, Home/End, or the controls. The first next action on a reveal slide shows its prediction answer. Buttons and form controls do not advance slides.
...
The supplied `zybook.pdf` remains private and Git-ignored. The materials use original teaching examples and diagrams. Students should access the licensed chapter through zyBooks or the LMS.
